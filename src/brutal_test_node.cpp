...
Solution3Node::Solution3Node(ros::NodeHandle nh, ros::NodeHandle pnh):
    ...
    if(map_name_ == "nctu") { 
        ...
        sub_gnss_ = nh.subscribe("gps", 250, &Solution3Node::gnss_cb2, this);
...
void Solution3Node::map_setup(void) {
    ...
    ROS_INFO("Loading pcd files of the map...");
...
void check_guess_finished(Solution3Node& node) {
    while(true) {
        ...
        cout << "Finished " << finished_index << " / " << node.guess_list_.size() << " quick ICP\r" << std::flush; // << endl;
...
void brutal_test(Solution3Node& node, Eigen::Matrix4d    input_guess_matrix, int cloud_start_idx, int kNumThreads, int kNumPretestFrames, int kNumPretestPoses, bool flag_renounce){
    // Create thread vector
    std::vector<std::thread> threads_list;
    
    node.guess_list_.clear();
    node.guess_matrix_ = input_guess_matrix;
    ...
    for(int i = 0; i < kNumPretestPoses; i++) {
        ...
        for(int j = cloud_start_idx; j < kNumPretestFrames + cloud_start_idx; j++)
    ...
    while(ros::ok()) {
        ...
        if(node.flag_init_guess_ == false) {
            ...
            if(node.cnt_imu_msgs_ > 0 && node.cnt_gps_msgs_ > 0 
                ...
                brutal_test(node, node.guess_matrix_, 0, kNumThreads, kNumPretestFrames, kNumPretestPoses, true);
            ...
            if(!node.clouds_list_.empty() && node.imu_list_.size() >= 2) {
                ...
                */
                ...
                brutal_test(node, node.guess_matrix_, 0, 4, 2, 100, false);
            }
        }
    }
    file.close();
